...
const App = () => {
  ...
  const filteredTokens = tokens.filter(t => 
    t.token_name.toLowerCase().includes(searchTerm.toLowerCase()) || 
    t.underlying_asset.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const theme = {
    ...
    tag: isDark ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-emerald-50 text-emerald-600 border-emerald-100',
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-[#c5a059] rounded-lg flex items-center justify-center">
              <TrendingUp className="text-[#0a0a0a] w-5 h-5" />
            </div>
            <h1 className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-[#c5a059]'}`}>
              {t[lang].navTitle}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button className={`flex items-center gap-1 px-4 py-2 border rounded-full text-sm font-medium ${isDark ? 'border-[#333] text-gray-300 hover:bg-[#222]' : 'border-[#c5a059]/30 text-[#c5a059] bg-[#c5a059]/10 hover:bg-[#c5a059]/20'}`}>
                <Twitter size={16} />
                <span>{t[lang].twitter}</span>
              </button>
              <button className={`flex items-center gap-1 px-4 py-2 rounded-full text-sm font-medium shadow-sm ${isDark ? 'bg-white text-black hover:bg-gray-200' : 'bg-[#c5a059] text-white hover:bg-[#b38f4d]'}`}>
                  ...
                  <tr key={token.id} className={`${theme.rowHover} transition-colors group`}>
                    <td className="px-6 py-6">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <span className={`font-bold text-lg ${theme.textMain}`}>{token.token_name}</span>
                          {token.status === 'Hot' && (
                        ...
                        <div className="flex flex-col mt-2 space-y-1">
                          ...
                          <div className={`flex items-center gap-1.5 text-xs ${theme.textSub}`}>
                            ...
                            <span>{t[lang].underlying}: <span className={isDark ? 'text-gray-200' : 'text-slate-700'}>{token.underlying_asset}</span></span>
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-6">
                      <div className="flex flex-col">
                        <span className={`text-xl font-bold font-mono tracking-tight ${isDark ? 'text-[#c5a059]' : 'text-amber-500'}`}>{token.price}</span>
                    ...
                    <td className="px-6 py-6">
                      <div className={`inline-flex items-center gap-1 px-3 py-1 rounded-lg border ${theme.valuationBg}`}>
                        <span className={`text-sm font-bold ${isDark ? 'text-gray-200' : 'text-slate-700'}`}>{token.implied_valuation}</span>
                      </div>
                    </td>

                    <td className="px-6 py-6">
                      <div className="flex flex-wrap gap-2">
                        {(lang === 'zh' ? token.remarks_zh : token.remarks_en).map((remark, idx) => (
                          <span 
                            key={idx} 
                    ...
                    <td className="px-6 py-6 text-right">
                      <a 
                        ...
                        className={`px-6 py-2.5 rounded-xl font-bold text-sm shadow-md active:scale-95 transition-all inline-flex items-center gap-2 ml-auto ${theme.btnBuy}`}
                      >
